...
def writefile1():
    ...
    if var1.get():
       ...
       filer = open(str(c_)+"result_fslda.txt","w")
    elif var2.get():
       a,b,c,d=trainsetfit2(Xtr,ytr)
       filer = open(str(c_)+"result_sfslda.txt","w")
    ...
    savename4= str(c_) + '_strtspred.csv'
...
def writefile3():   
    ...
    if ytr5.columns[0] in file3.columns: 
       ...
       dfsvd=pd.concat([nvd,Xvd,yvd,yprvd,yprvd2],axis=1)
       dfsvd['Set'] = 'Validation'
       #findv=pd.concat([dfstr,dfsts],axis=0)
       #finda.to_csv('find.csv',index=False)
       #savename4 = filedialog.asksaveasfilename(initialdir=initialdir,title = "Save File with Predicted Activity")
       savename4= str(e_) + '_vdpred.csv'
       dfsvd.to_csv(savename4,index=False)
       filer2 = open(str(e_)+"_vdpred.txt","w")
       ...
       yprvd2=pd.DataFrame(model.predict_proba(Xvd))
       yprvd2.columns=[['%Prob(-1)','%Prob(+1)']]
       dfsvd=pd.concat([nvd,Xvd,yprvd,yprvd2],axis=1)
       dfsvd['Set'] = 'Screening'
       #findv=pd.concat([dfstr,dfsts],axis=0)
       #finda.to_csv('find.csv',index=False)
       #savename4 = filedialog.asksaveasfilename(initialdir=initialdir,title = "Save File with Predicted Activity")
...
def ycrand(): 
    s=file4.iloc[:,-1:]
    df=pd.concat([file7,s],axis=1)
    #dft=df[df['Set']=='Sub_train']
    nc=seventhEntryTabFive.get()
    nc=int(nc)
    #nc=len(desc)
    Xtr5=file4.iloc[:,1:nc+1]
    desc=Xtr5.columns
    ni=ninthEntryTabFive.get()
    ni=int(ni)
    a=ycrandom(df,nc,desc,ni)
    filer3 = open("_ycresult.txt","w")
    filer3.write('The yclambda value after '+str(ni)+' run is '+str(a.randomization()))
    
    
form = tk.Tk()
form.title("QSAR-Co Extension")
...
tab_parent.add(tab1, text="Select and split dataset")
tab_parent.add(tab2, text="Box Jenkins algorithm")
#tab_parent.add(tab4, text="Model development")
tab_parent.add(tab3, text="Screening/validation of Data")


# === WIDGETS FOR TAB ONE
#first
firstLabelTabOne = tk.Label(tab1, text="Select Data",font=("Helvetica", 12))
firstLabelTabOne.place(x=60,y=10)
firstEntryTabOne = tk.Entry(tab1,text='',width=40)
firstEntryTabOne.place(x=150,y=13)
b5=tk.Button(tab1,text='Browse', command=data,font=("Helvetica", 10))
b5.place(x=400,y=10)


secondLabelTabOne_1=Label(tab1, text='Dataset division before Box-Jenkins',font=('Helvetica 12 bold'))
secondLabelTabOne_1.place(x=150,y=55)
secondLabelTabOne=Label(tab1, text='%data-points in validation set',font=("Helvetica", 12), justify='center')
secondLabelTabOne.place(x=80,y=80)
secondEntryTabOne=Entry(tab1)
secondEntryTabOne.place(x=300,y=85)


thirdLabelTabOne=Label(tab1, text='Seed value',font=("Helvetica", 12))
thirdLabelTabOne.place(x=200,y=115)
thirdEntryTabOne=Entry(tab1)
thirdEntryTabOne.place(x=300,y=115)

b6=tk.Button(tab1, text='Solution', bg="orange", command=solsave,font=("Helvetica", 10))
b6.place(x=280,y=145)

firstLabelTabTwo = tk.Label(tab1, text="Box Jenkins based moving average",font=('Helvetica 12 bold'))
firstLabelTabTwo.place(x=180,y=180)
#secondLabelTabOne = tk.Label(tab1, text="Correlation cutoff")
#jobLabelTabOne = tk.Label(tab1, text="Address:")

firstLabelTabTwo_1=tk.Label(tab1, text='Number of conditions',font=("Helvetica", 12))
firstLabelTabTwo_1.place(x=150,y=215)
firstEntryTabTwo = tk.Entry(tab1)
firstEntryTabTwo.place(x=310,y=215)

secondLabelTabTwo=Label(tab1, text='% data-points in test set',font=("Helvetica", 12))
secondLabelTabTwo.place(x=130,y=240)
...
b7=tk.Button(tab1, text='Solution', bg="orange", command=solsave2,font=("Helvetica", 10))
b7.place(x=280,y=290)
#################################
...
b1=Button(tab2, text='Solution', command=writefile1,bg="orange",font=("Helvetica", 10))
...
b2=Button(tab2, text='Solution', command=writefile1,bg="orange",font=("Helvetica", 10),anchor=W, justify=LEFT)
...
b10=Button(tab3, text='Solution', command=writefile3,bg="orange",font=("Helvetica", 10),anchor=W, justify=LEFT)
b10.place(x=350,y=115)

forthLabelTabFive = tk.Label(tab3, text="Yc randomization(First import training set result file)",font=('Helvetica 12 bold'),anchor=W, justify=LEFT)
forthLabelTabFive.place(x=110,y=150)


seventhLabelTabFive = tk.Label(tab3, text="Number of conditions",font=("Helvetica", 12),anchor=W, justify=LEFT)
seventhLabelTabFive.place(x=150,y=210)
seventhEntryTabFive = tk.Entry(tab3)
seventhEntryTabFive.place(x=310,y=210)


sixthLabelTabFive = tk.Label(tab3, text="Open training set file",font=('Helvetica 12'),anchor=W, justify=LEFT)
sixthLabelTabFive.place(x=90,y=180)
sixthEntryTabFive = tk.Entry(tab3, width=40)
sixthEntryTabFive.place(x=250,y=180)
b11=tk.Button(tab3,text='Browse', command=data5)
b11.place(x=500,y=180)
...
ninthLabelTabFive.place(x=195,y=250)
ninthEntryTabFive=Entry(tab3)
ninthEntryTabFive.place(x=310,y=250)

b10=Button(tab3, text='Solution', command=ycrand,bg="orange",font=("Helvetica", 10),anchor=W, justify=LEFT)
b10.place(x=350,y=280)
